...
- **The app prepares a section's data.** Each section's generate job used to be
  ...
  now reads the job's run history: any successful run means the data exists (a
  CLI run leaves the same history, so data generated by hand counts), an active
  run shows as "preparing", and a job that has never run is started by the app
  on first open. The app's principal holds CAN_MANAGE_RUN on the 17 generate
  jobs through the bundle, so no Unity Catalog grant is needed for the check.
