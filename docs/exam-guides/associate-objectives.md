...
- **`ASSOC-S2-O4`** — Configure Lakeflow Connect to reliably ingest data from diverse enterprise sources into Unity Catalog-governed tables.
  ...
  - Standard connectors should be fine, but managed connectors target SaaS systems (Salesforce, Workday, ServiceNow) that a learner is unlikely to have. Confirm which connectors Free Edition exposes before deciding how much of this can be hands-on.
- **`ASSOC-S2-O5`** — Use JDBC/ODBC or REST clients in notebooks to land data into cloud storage or directly into Unity Catalog-governed tables, usually orchestrated and scheduled with Lakeflow Jobs.
  ...
  - Needs an external database to read from, and Free Edition restricts egress to trusted domains. Promising workaround: use the account's own Lakebase Postgres project as the JDBC source, keeping the lab entirely inside Databricks. Verify Lakebase JDBC connectivity from a serverless notebook before writing this.
