...
# MAGIC %md
# MAGIC # Grade the PRO-S5 assignment
# MAGIC
...
workdir = tempfile.mkdtemp(prefix="grade_pro_s5_")
shutil.copytree(os.path.join(assignment_dir, "tests"), os.path.join(workdir, "tests"))
fixtures = os.path.join(workdir, "expected.json")
shutil.copyfile(os.path.join(repo_root, "solutions", "PRO-S5", "expected.json"), fixtures)
os.environ["PRO_S5_FIXTURES"] = fixtures
...
dbutils.fs.put("/Volumes/workspace/de_prep/raw/_grading/PRO-S5-report.txt",
               report[-30000:] + f"\n\nexit={exit_code}\n", overwrite=True)
