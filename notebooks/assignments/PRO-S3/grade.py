...
# MAGIC %md
# MAGIC # Grade the PRO-S3 assignment
# MAGIC
...
workdir = tempfile.mkdtemp(prefix="grade_pro_s3_")
shutil.copytree(os.path.join(assignment_dir, "tests"), os.path.join(workdir, "tests"))
fixtures = os.path.join(workdir, "expected.json")
shutil.copyfile(os.path.join(repo_root, "solutions", "PRO-S3", "expected.json"), fixtures)
os.environ["PRO_S3_FIXTURES"] = fixtures
...
dbutils.fs.put("/Volumes/workspace/de_prep/raw/_grading/PRO-S3-report.txt",
               report[-30000:] + f"\n\nexit={exit_code}\n", overwrite=True)
