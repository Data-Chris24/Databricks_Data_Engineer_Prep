...
# MAGIC %md
# MAGIC # Grade the ASSOC-S7 assignment
# MAGIC
...
workdir = tempfile.mkdtemp(prefix="grade_assoc_s7_")
shutil.copytree(os.path.join(assignment_dir, "tests"), os.path.join(workdir, "tests"))
fixtures = os.path.join(workdir, "expected.json")
shutil.copyfile(os.path.join(repo_root, "solutions", "ASSOC-S7", "expected.json"), fixtures)
os.environ["ASSOC_S7_FIXTURES"] = fixtures
...
dbutils.fs.put("/Volumes/workspace/de_prep/raw/_grading/ASSOC-S7-report.txt",
               report[-30000:] + f"\n\nexit={exit_code}\n", overwrite=True)
