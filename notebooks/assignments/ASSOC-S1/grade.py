...
# MAGIC %md
# MAGIC # Grade the ASSOC-S1 assignment
# MAGIC
...
workdir = tempfile.mkdtemp(prefix="grade_assoc_s1_")
shutil.copytree(os.path.join(assignment_dir, "tests"), os.path.join(workdir, "tests"))
fixtures = os.path.join(workdir, "expected.json")
shutil.copyfile(os.path.join(repo_root, "solutions", "ASSOC-S1", "expected.json"), fixtures)
os.environ["ASSOC_S1_FIXTURES"] = fixtures
...
dbutils.fs.put("/Volumes/workspace/de_prep/raw/_grading/ASSOC-S1-report.txt",
               report[-30000:] + f"\n\nexit={exit_code}\n", overwrite=True)
